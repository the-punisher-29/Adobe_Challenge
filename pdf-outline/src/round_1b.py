...
# def select_top_headings(headings_str, job, top_n=6):
#     # ... (as per your rank_headings_with_embeddings logic)
#     model = SentenceTransformer("all-MiniLM-L6-v2")
#     documents = []
#     for doc_block in headings_str.strip().split("\n\n"):
#         if "Title:" not in doc_block or "Headings:" not in doc_block:
...
#         ranked = sorted(zip(headings, similarities.tolist()), key=lambda x: -x[1])
#         top_headings = ranked[:top_n]
#         results.append({"title": title, "top_headings": [h for h, _ in top_headings]})
#     return results

# def build_qwen_prompt(top_headings_by_doc, persona, job):
#     # Compose a prompt for Qwen
#     docs_joined = ""
#     for doc in top_headings_by_doc:
#         docs_joined += (
#             f"\nTitle: {doc['title']}\nHeadings: " + ", ".join(doc["top_headings"])
#         )
#     prompt = (
#         f"You are a {persona}. Your task is '{job}'.\n"
#         "Given the following document titles and their most relevant headings, "
#         "select and rank the most relevant headings for the task. "
#         "Output a json like: "
#         "[{'title': ..., 'ranked_headings': [ ... ] }]\n"
#         "The headings and titles are as follows: \n"
#         f"{docs_joined}"
#     )
#     return prompt
...
#     top_headings = select_top_headings(headings, job=job_to_be_done, top_n=6)
#     prompt = build_qwen_prompt(top_headings, persona, job_to_be_done)
#     print("\nPrompt to Qwen:\n", prompt)
